...
inline int32_t BinaryTriangleR3::Split2(BinTriPool *pool)
...
void BinaryTriangleR3::TestSplit(LodTree *lod, int32_t variance, int32_t LimitLod, int32_t level, int32_t index, BinTriPool *pool)
{
...
void BinaryTriangleR3::TestSplitZ(int32_t level, int32_t LimitLod, int32_t index, float cz1, float cz2, float cz3, LodTree *lod, BinTriPool *pool)
{
...
void BinaryTriangleR3::TestSplitClip(int32_t level, float variance, int32_t LimitLod, int32_t index, float radius, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, float lclip[3], float rclip[3], float zclip[3], LodTree *lod, BinTriPool *pool)
{
...
inline void GLRenderEngine3::BinTriVert(int32_t x, int32_t y){
...
void GLRenderEngine3::RenderBinTri(BinaryTriangleR3 *btri, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3){
...
inline void GLRenderEngine3::AddFanPoint(int32_t x, int32_t y, float h){
	FanStack[nFanStack] = x;
	FanStack[nFanStack + 1] = y;
	FanStackf[nFanStack + 2] = h;
	nFanStack += 3;
}
inline void GLRenderEngine3::AddFanPoint(int32_t x1, int32_t y1, float h1, int32_t x2, int32_t y2, float h2, int32_t x3, int32_t y3, float h3){
...
void GLRenderEngine3::RenderBinTriFan(BinaryTriangleR3 *btri, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, int32_t sense, float h1, float h2, float h3){
	if(btri->LeftChild && btri->RightChild){
		int32_t avgx = (x1 + x2) >>1, avgy = (y1 + y2) >>1;
		float th;
		if(btri->iheight){
...
bool GLRenderEngine3::GLTerrainRender(Terrain *map, Camera *cam, int32_t flags, float quality, int32_t ms){
	//
	//
...
//	GLfloat fogcol[4] = {0.7f, 0.8f, 0.9f, 0.0f};
	...
	int32_t LimitLod = std::max(0.0f, (1.0f - InputQuality) * 4.0f);	//Fix limit lod based on specified virtual quality, to reduce lod oscillations.
	//
	if((flags & GLREND_LOCKPATCHES) == 0){
...
bool GLRenderEngine3::GLRenderWater(Terrain *map, Camera *cam, int32_t flags, float quality){
	if(!patches) return false;
	if(!map || !cam){
