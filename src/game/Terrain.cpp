...
inline int32_t Clip(int32_t n, int32_t low, int32_t high){ return (n < low ? low : (n > high ? high : n)); }
...
Terrain::Terrain(){
	...
	data32 = NULL;
//	ShadeLookup32 = NULL;
//	for(int32_t i = 0; i < NUM_SQRT; i++){
//		SqrtLookup[i] = (int32_t)(sqrt((double)(i <<FP)) * FPVAL);
//	}
...
void Terrain::SetScorchEco(int32_t i){
	ScorchTex = i;
}
int32_t Terrain::GetScorchEco(){
	return ScorchTex;
}
bool Terrain::Load(const char *name, int32_t *numecoptr, EcoSystem *eco, int32_t maxeco, bool mirror){
	IFF iff;
	if(name){
...
bool Terrain::Load(IFF *iff, int32_t *numecoptr, EcoSystem *eco, int32_t maxeco, bool mirror){
	...
	if(iff->IsFileOpen() && iff->IsFileRead()){
		if(iff->IsType("VOXL")){
			...
			if(iff->FindChunk("ECOS") >= 8 && numecoptr && eco && maxeco > 0){
				int32_t numeco = iff->ReadLong();
				numeco = std::min(numeco, maxeco);
				*numecoptr = numeco;
				int32_t floats = iff->ReadLong();	//floats per eco.
				uchar slen;
				if(floats >= 6){
					for(i = 0; i < numeco; i++){
...
bool Terrain::Save(const char *name, EcoSystem *eco, int32_t numeco){
	IFF iff;
	if(name){
...
bool Terrain::Save(IFF *iff, EcoSystem *eco, int32_t numeco){
...
int32_t Terrain::FastSqrt(int32_t x, int32_t y, int32_t z){
	return 1;//SqrtLookup[std::max(std::min((abs(x * x) + abs(y * y) + abs(z * z)) >>FP, 0), NUM_SQRT - 1)];
}
bool Terrain::Init(int32_t x, int32_t y){
	Free();
...
int32_t Terrain::Blow(unsigned char *dest, int32_t dw, int32_t dh, int32_t dp,
		 int32_t sx, int32_t sy, int32_t sx2, int32_t sy2, int32_t flag, int32_t destbpp, PaletteEntry pe[256]){
	//Check for sanity.
	...
	if(destbpp == 32){
		...
		int32_t tdestp = dp - (sx2 - sx);	//Bytes to add to dest each line.
		unsigned char *tsrc = (unsigned char*)data + flag + sx * 2 + sy * width * 2;
		int32_t tsrcp = width * 2 - (sx2 - sx) * 2;
		int32_t x, y;
		for(y = sy; y < sy2; y++){
			for(x = sx; x < sx2; x++){
				*tdest++ = *tsrc;
...
//	AvgRng ar;
	int32_t first = 1, x = 0, y = 0, step = 0, i, t;//, tx, ty
	char buf[1000];
	int32_t randrange;
...
if(Form2 == TFORM_FRACTAL){
	...
	while(level >= 0){
		...
		if(first){	//Seed terrain with mini-map created values.
			...
			int32_t xflip, yflip = 0, random;	//flip-flop variables.
			for(y = 0; y < height; y += step){
...
if(Form2 == TFORM_SPLINE){
	...
	int32_t hy1, hy1V, hy1VD;
	int32_t hy2, hy2V, hy2VD;
	int32_t vy, vyV, vyVD;
	int32_t x2, y2, w2 = step / 2;
	...
	for(y2 = 0; y2 < height / step; y2++){
		for(x2 = 0; x2 < width / step; x2++){
			...
			hy1V = 0;
			hy1 = ((int32_t)*(MiniD + x2 + y2 * MiniW)) <<FP2;
			hy1VD = ((*(MiniD + x2 + 1 + y2 * MiniW) <<FP2) - hy1) / (w2 * w2);
			//Lower spline for this box.
			hy2V = 0;
			hy2 = ((int32_t)*(MiniD + x2 + (y2 + 1) * MiniW)) <<FP2;
			hy2VD = ((((int32_t)*(MiniD + x2 + 1 + (y2 + 1) * MiniW)) <<FP2) - hy2) / (w2 * w2);
	...
	if(Average){
		if(Normalize){	//Normalizing is SLOOW.
			...
			int32_t yrange1 = max - min, yrange2 = realmax - realmin, vtot, vnum, ix, iy;
	...
	if(Perturb > 0){
		int32_t nump = ((width * height) / 10) * Perturb;
		for(i = 0; i < nump; i++){
			x = rand() % width;
			y = rand() % height;
			SetH(x, y, GetH(x, y) + 1);
...
int32_t Terrain::EcoTexture(EcoSystem *eco, int32_t numeco, int32_t x, int32_t y){
	if(x < width && y < height && x >= 0 && y >= 0 && data && eco){
		int32_t T = 0, H = GetHraw(x, y);
...
bool Terrain::Texture(EcoSystem *eco, int32_t numeco, bool UseIDMap, int32_t x1, int32_t y1, int32_t x2, int32_t y2){
	...
	if(x2 == 0) x2 = width; 
	if(y2 == 0) y2 = height;	//Default to whole shebang.
	x1 = std::max(0, x1); y1 = std::max(0, y1);	//Clip.
	x2 = std::min(width, x2); y2 = std::min(height, y2);	//Combined with sanity, all should be OK.
	int32_t x, y, T = 0, H;
	double S;
	int32_t xmask[50], ymask[50];	//NOTE, textures must be power of two width and height now!
	char EcoOk[50];
	for(int32_t e = 0; e < numeco; e++){
...
bool Terrain::Lightsource(int32_t x1, int32_t y1, int32_t x2, int32_t y2){
	...
	for(y = y1; y < y2; y++){
		for(x = x1; x < x2; x++){
			i = NUM_SHADE / 2 + (int32_t)((double)NUM_SHADE / 2.0 * GetSlope(x, y, LightX, LightY, LightZ));	//Light intensity.
			SetCraw(x, y, ShadeLookup[GetCraw(x, y)][std::min(std::max(i, 0), NUM_SHADE - 1)]);
		}
	}
	return true;
}
int32_t Terrain::ClearCMap(unsigned char val){
	if(data && width > 0 && height > 0){
		for(int32_t y = 0; y < height; y++){
...
bool Terrain::TextureLight32(EcoSystem *eco, int32_t numeco, int32_t x1, int32_t y1, int32_t x2, int32_t y2, bool UseShadeMap){
	if(!data32){
		return false;
...
//	float fi;
	...
	int32_t xmask[32], ymask[32], xshift[32];	//NOTE, textures must be power of two width and height now!
	char EcoOk[32];
	for(int32_t e = 0; e < numeco; e++){
		xmask[e] = (1 << HiBit(eco[e].tex->Width())) - 1;
		...
		for(x = x1; x < x2; x++){
			int32_t i, T;
			{
				...
				if(UseShadeMap){
					...
					i = (int32_t)((amb + ((float)(NUM_SHADE - 1)) * invamb * std::min(std::max(slope, 0.0f), 1.0f)));	//Light intensity.
				}
			}
			...
			if(ScorchTex >= 0 && EcoOk[ScorchTex]){
				...
				int32_t foo = ((x & widthmask) <<2);
				int32_t si = ((255 - (int32_t)GetCwrap(x, y)) * (NUM_SHADE)) >>8;	//Alpha blend to special scorch texture rather than to black.
			...
			if(EcoOk[T]){
				if(ScorchTex >= 0 && EcoOk[ScorchTex]){
					...
					register int32_t foo = ((x & widthmask) <<2);
					int32_t si = ((255 - (int32_t)GetCwrap(x, y)) * (NUM_SHADE)) >>8;	//Alpha blend to special scorch texture rather than to black.
...
inline float Terrain::GetSlope(int32_t x, int32_t y, float lx, float ly, float lz){
	static float SlopeTab[SLOPETAB_SIZE][SLOPETAB_SIZE][3];
	static int32_t TabMade = 0;
	float *tflt;
	int32_t s1, s2;
	if(!TabMade){
...
inline unsigned short Terrain::GetBigH(int32_t x, int32_t y){
...
inline bool Terrain::SetBigH(int32_t x, int32_t y, unsigned short v){
...
float Terrain::FGetI(float x, float y){
	return (float)GetI((int32_t)(x * FPVAL), (int32_t)(y * FPVAL)) * INVFPVAL;/// (float)FPVAL;
}
inline AvgRng Terrain::Average(int32_t x, int32_t y, int32_t p){
	AvgRng ar;
	...
	for(int32_t yy = y - step; yy <= y + step; yy += step){
		if(yy >= 0 && yy < height){
			for(int32_t xx = x - step; xx <= x + step; xx += step){
...
bool Terrain::MakeShadeLookup(InversePal *inv){//PALETTEENTRY *pe){
	...
	int32_t r, g, b, c, i, col;//, k, diff, col, t;

	int32_t floor = 0; //NUM_SHADE;
//	int32_t floor = NUM_SHADE / 3;	//Boost "ambient" color a bit.
//	int32_t floor = (int32_t)((Ambient * (float)NUM_SHADE) / (1.0f - Ambient));// / 3;	//Boost "ambient" color a bit.
